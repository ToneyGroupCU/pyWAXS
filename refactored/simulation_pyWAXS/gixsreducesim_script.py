...
def metadataminer(samplename, datapath, keylist, basetime, index):
    ...
    thpos = file_dict['thpos']
    thpos = re.findall("\d+\.\d+", thpos)
    thpos = thpos[0]
    file_dict['thpos'] = thpos

    exptime = file_dict['exptime']
    exptime = re.findall("\d+\.\d+", exptime)
    exptime = str(np.round(float(exptime[0]),1)) # round exposure time to nearest decimal place
    ...
    scanid = file_dict['scanid']
    dict_entryname = samplename + '_' + scanid + '_' + str(totaltime)
...
def format2drecip(data_recip, qxy, qz, datafile_dict, datapath, recipmap2d_path, timestamp):
    ...
    imgheader = solutionnum + ' | Time: ' + totaltime
    plt.title(imgheader, fontsize = 30) # title
...
def giwaxs_reduce(datafolder, samplename, qparams, chiparams, poleparams, peakparams, corrections, keylist, poni_file, mask_file, globStr = '*s_[0-9][0-9][0-9][0-9][0-9][0-9]_[0-9][0-9][0-9][0-9][0-9][0-9]_maxs.tiff'):
